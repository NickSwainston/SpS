...
def cand_plot(pdf, cand, pulses, pulses_all, events, meta_data):
  ...
  gs = gridspec.GridSpec(3, 6, wspace=1., hspace=.5)
  ...
  # DM vs Time
  ax2 = plt.subplot(gs.new_subplotspec((0,2), 2, 5))
  scatter_beam(ax2, pulses, pulses_all, cand)

  # S/N vs DM
  if not pulses_all.empty: 
...
def meta_data_plot(ax, pulses, cand, meta_data):
  ...
  if cand.N_pulses == 1:
    ...
    ax.annotate('Downsampled (factor): {0:.0f}'.format(pulses.Downfact.iloc[0]), xy=(0,2))
    ...
    ax.annotate('N. pulses: {0:.0f}'.format(cand.N_pulses), xy=(0,2))
...
def scatter_SNR(ax, events_beam, cand):
  ...
  ev = events_beam[np.abs(events_beam.DM - cand.DM) < 3]

  # Plot all the events
  ax.scatter(ev.DM, ev.Sigma, c='k', s=5, linewidths=[0.,], zorder=2)
...
def puls_meta_data(ax, puls, idx, i):
  ...
  ax.axis([0,10,0,8])
  ax.annotate("Pulse n. {}".format(i), xy=(0,7)) 
  ax.annotate('Pulse code: {}'.format(idx), xy=(0,6))
  ax.annotate('DM (pc cm$^{{-3}}$): {0:.2f}'.format(puls.DM), xy=(0,5))
  ax.annotate('Time (s): {0:.2f}'.format(puls.Time), xy=(0,4))
  #ax.annotate('dDM (pc cm$^{-3}$): {0:.2f}'.format(puls.dDM), xy=(0,6))
  ax.annotate('Sigma: {0:.1f}'.format(puls.Sigma), xy=(0,3))
  ax.annotate('Downsampled (factor): {0:.0f}'.format(puls.Downfact), xy=(0,2))
  ax.annotate('N events: {0:.0f}'.format(puls.N_events), xy=(0,1))
...
def puls_DM_Time(ax, event, all_events, puls):
  ...
  dDM = event.DM.max() - event.DM.min()
  dt = event.Time.max() - event.Time.min()
  DM_min = puls.DM - dDM * 3.
  DM_max = puls.DM + dDM * 3.
  Time_min = puls.Time - dt * 3.
  Time_max = puls.Time + dt * 3.
...
def puls_SNR_DM(ax, event):
  ...
  ax2.plot(event.DM, event.Downfact, 'r')
  ax2.set_ylabel('Downsampled (factor)', color='r')
...
def puls_dynSpec(ax1, ax2, puls, args, prof_ax=None):
  ...
  ds, nbinsextra, nbins, start = waterfaller.waterfall(psrfits_file, start, duration, nsub=nsub, dm=puls.DM,  downsamp=nsub,\
      #width_bins=df, \
  ...
  ds, nbinsextra, nbins, start = waterfaller.waterfall(psrfits_file, start, duration, nsub=nsub, dm=0., downsamp=nsub, \
